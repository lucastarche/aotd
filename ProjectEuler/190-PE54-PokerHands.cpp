...
int royalFlush(const Hand& h) {
    if (flush(h) > 0 && straight(h) == 14)
        return 14;
    return -1;
}

int straightFlush(const Hand& h) {
    if (flush(h) > 0 && straight(h) > 0)
        return flush(h);
    return -1;
}
...
int fullHouse(const Hand& h) {
    if (threeOfAKind(h) > 0 && onePair(h) > 0)
        return max(threeOfAKind(h), onePair(h));
    return -1;
}

bool firstPlayerWon(const Hand& a, const Hand& b) {
    if (royalFlush(a) != royalFlush(b))
        return royalFlush(a) > royalFlush(b);
    if (straightFlush(a) != straightFlush(b))
        return straightFlush(a) > straightFlush(b);
    if (fourOfAKind(a) != fourOfAKind(b))
        return fourOfAKind(a) > fourOfAKind(b);
    ...
    if (twoPairs(a) != twoPairs(b))
        return twoPairs(a) > twoPairs(b);
    if (onePair(a) != onePair(b))
        return onePair(a) > onePair(b);
...
long long solve(const vector<pair<Hand, Hand>>& games) {
    long long ans = 0LL;
    for (auto& game : games) {
        if (firstPlayerWon(game.first, game.second))
            ans++;
    }
    return ans;
}
