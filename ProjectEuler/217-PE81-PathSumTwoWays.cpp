//PE 81: Path Sum: Two ways
//Problem: Given an 80x80 matrix, find the path with the smallest sum from the top left to the bottom right, if you can only go down and right
//Solution: Today's (just like 82 and 83) can be solved by using Dijkstra's algorithm in an implicit graph.
//Runtime: O(n^2 log n)
...
const vector<long long> dx = { 1, 0 };
const vector<long long> dy = { 0, 1 };
