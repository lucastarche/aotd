...
string parseEnglish(int n) {
    if (n < 0) return "minus " + parseEnglish(-n);
    if (n < 20) return numString[n];
    if (n <= 90 && n % 10 == 0) return numString[n];
    if (n < 100) return numString[(int)(n / 10) * 10] + "-" + numString[n % 10];
    if (n <= 900 && n % 100 == 0) return numString[n / 100] + " " + numString[100];
    if (n < 1000) return parseEnglish((int)(n / 100) * 100) + " and " + parseEnglish(n % 100);
